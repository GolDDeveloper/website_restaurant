...
<br>
<label for="tisch">Tischnummer</label>
<input type="number" id="tisch" min="0">
<br>
...
function checkPreis() {
  ...
  var $fixed = $pr.toFixed(2);
  document.getElementById("preis").innerText = "Aktueller Preis: " + $fixed + "€";
}
...
function resetSeite() {
  ...
  document.getElementById("tisch").value="";
  checkPreis();
  checkStatus();
}
...
var $tisch = document.getElementById("tisch").value;
  ...
  if ($tisch < 1) {
    alert("Bitte geben Sie ihre Tischnummer ein.");
  }
  else {
    for (var $i = 0; $i < 34; $i++) {
      if ($bestellung[$i] !== 0) {
  ...
  // Insert Bestellung in Datenbank
